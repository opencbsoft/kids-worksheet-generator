...
class Main(Generator):
	name = 'Completeaza numerele lipsa usor'
	years = [5, 6]
		...
		for i in range(self.count):
			...
			for index in range(10):
				...
				while len(missing) < target_missing:
					random_position = random.randint(index*10+2, (index+1)*10-1)
					if random_position not in missing and random_position-1 not in missing and random_position+1 not in missing:
						if len(result) > 10:
							upper = random_position - 10
							if result[upper-1] != "":
